...
module.exports = {
    ...
    async execute(oldMember, newMember) {
        ...
        if(config.channels.welcome[oldMember.guild.id]) {
            if(guild.roles.cache.get(config.channels.welcome[guild.id].memberRole)) {
                if(oldMember.pending === true && newMember.pending === false) {
                    oldMember.roles.add(config.channels.welcome[guild.id].memberRole, 'User Joined (Discord Verification Respected)')
                }
            }
        }
    }
